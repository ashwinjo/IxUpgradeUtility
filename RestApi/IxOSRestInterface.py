...
class IxRestSession(object):
    ...
    def get_ixos_admin_uri(self):
        return 'https://%s/chassis/api/v2/admin' % self.chassis_ip
        
    def get_packages(self):
        return self.http_request(
            'GET',
            self.get_ixos_admin_uri() + '/applications/uploads'
        ).json()

    def add_target_chassis_to_controller_chassis(self, target_chassis_list=None):
        d = []
        for ip in target_chassis_list:
            d.append({"address": ip})
        return self.http_request(
            'POST',
            self.get_ixos_admin_uri() + '/bulk/updates/systems/operations/addmultiple',
            payload = {"systems":d}
        )
        
    def authenticate_target_chassis(self, list_target_chassis_only_dict=None):
       for chassis in list_target_chassis_only_dict:
            return self.http_request(
                'POST',
                self.get_ixos_admin_uri() + '/bulk/updates/systems/'+str(chassis.get('id'))+'/operations/authenticate',
                payload = {"username":chassis.get('username'), "password":chassis.get('password')}
            )
    
    def get_target_chassis_information(self):
        return self.http_request(
            'GET',
            self.get_ixos_admin_uri() + '/bulk/updates/systems'
        ).json()
    ...
    def upgrade_target_chassis(self, uploadId=None, list_target_chassis_only_dict=None):
        for chassis in list_target_chassis_only_dict:
            ...
            out =  self.http_request(
                'POST',
                self.get_ixos_admin_uri() + '/bulk/updates/systems/operations/install',
                payload = {
                                "uploadId": uploadId,
                                "onlyUpload": False,
                                "systems": [
    ...
    def extract_and_accept_eula_on_target_chassis(self, uploadId=None):
        # Extract EULA ID
        out =  self.http_request(
                'POST',
                self.get_ixos_admin_uri() + '/bulk/updates/eula/operations/extract',
                payload = {"uploadId":uploadId}
            )
        
        eulaId = self.http_request('GET', out).json()['eulaId']
        asyncId = self.http_request('GET', out).json()['asyncId']
        
        # Sign EULA
        out =  self.http_request(
                'POST',
                self.get_ixos_admin_uri() + '/bulk/updates/eula/operations/accept',
                payload = {"asyncId":asyncId,
                           "eulaId":eulaId}
            )
